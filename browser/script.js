...
function carregarBasesSimuladas() {
  ...
  const tipos = ["VAN", "CAMINHÃO", "CARRO", "SUV", "MOTO", "VUC"];
  const statusList = ["EM ROTA", "PARADO", "EM MANUTENÇÃO", "ALERTA"];

  for (let i = 1; i <= 132; i++) {
    const id = `RJ-${404 + i}-${i.toString().padStart(2, '0')}`;
    const tipo = tipos[Math.floor(Math.random() * tipos.length)];
    const status = statusList[Math.floor(Math.random() * statusList.length)];
    
    // Coordenadas aleatórias ao redor do Rio de Janeiro
    const lat = -22.9068 + (Math.random() - 0.5) * 0.2;
    const lng = -43.1729 + (Math.random() - 0.5) * 0.3;

    BASE_VEICULOS_SIMULADOS.push(criarVeiculoSimulado(id, lat, lng, tipo, status));
  }
